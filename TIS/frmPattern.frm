...
Begin VB.Form frmPattern 
   BackColor       =   &H00808080&
   Caption         =   "Tread Pattern Data Entry"
   ClientHeight    =   5760
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8685
...
Adodc2.Refresh
Adodc2.Recordset.Find "Pattern=" & "'" & txtPattern & "'"

If Len(txtPattern) > 0 Then
    If Len(txtPattern2) = 0 Then
        Adodc1.Recordset.UpdateBatch
        savecancel
        delete
    Else
        mb = MsgBox("Tread Pattern already in used.", vbCritical, "Yokohama")
        txtPattern.SetFocus
        SendKeys "{Home}+{End}"
    End If
Else
